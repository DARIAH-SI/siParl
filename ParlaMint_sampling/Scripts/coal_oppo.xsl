...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Ignore parties older than this date; the date is set to the beginning of the Term 3-->
  <xsl:param name="cutoffDate">2000-10-27</xsl:param>
...
<!--      Unused code (coalition/opposition script)-->


<!-- <xsl:if test="$name = 'coalition'">
	<xsl:variable name="parties" select="concat(@mutual, ' ')"/>
	<xsl:variable name="from" select="@from"/>
	<xsl:variable name="to" select="@to"/>
	<xsl:variable name="opposition">
	  <xsl:for-each select="ancestor::tei:listOrg/tei:org">
	    <xsl:message select="concat('TEST: ', @xml:id)"/>
	    <xsl:if test="not(contains($parties, concat(@xml:id, ' ')))">
	      <xsl:variable name="org_role" select="@role"/>
	      <xsl:variable name="org_from" select="tei:event[tei:label = 'existence']/@from"/>
	      <xsl:variable name="org_to" select="tei:event[tei:label = 'existence']/@to"/>
	      <xsl:if test="(not($org_from) or $org_from &lt;= $to) 
			    and ($org_to &gt;= $from)
			    and (not($org_role='parliament' 
			    or $org_role='ethnic_communities' 
			    or $org_role='government'))">
		<xsl:value-of select="concat('#', @xml:id, ' ')"/>
		<xsl:message select="concat('INFO: ',  $org_from, ' - ', $org_to, ' / ', $from, ' - ', $to )"/>
	      </xsl:if>
	    </xsl:if>
	  </xsl:for-each>
	</xsl:variable>
	<relation name="opposition">
	  <xsl:attribute name="active" select="normalize-space($opposition)"/>
	...
	<xsl:message select="concat('INFO: Inserting opposition ', 
			     $opposition, ' from coalition ',
			     @from, ' - ', @to)"/>
      </xsl:if>
    </xsl:if>-->


<!--<xsl:template match="tei:listRelation//tei:relation[@name='coalition']" mode="relations">
  <xsl:variable name="parties" select="concat(@mutual, ' ')"/>
  <xsl:variable name="from" select="@from"/>
  <xsl:variable name="to" select="@to"/>
  <xsl:variable name="opposition">
    <xsl:for-each select="ancestor::tei:listOrg/tei:org">
      <xsl:message select="concat('INFOx: ', @role)"/>
      <xsl:if test="not(contains($parties, concat(@xml:id, ' ')))">
	<xsl:variable name="org_from" select="tei:event[tei:label = 'existence']/@from"/>
	<xsl:variable name="org_to" select="tei:event[tei:label = 'existence']/@to"/>
	<xsl:if test="(not($org_from) or $org_from &lt;= $to) and (not($org_to) or $org_to &gt;= $from)">
	  <xsl:value-of select="concat('#', @xml:id, ' ')"/>
	  <xsl:message select="concat('INFOx: ', $org_from, ' - ', $org_to, ' / ', $from, ' - ', $to )"/>
	</xsl:if>
      </xsl:if>
    </xsl:for-each>
  </xsl:variable>
